...
import com.binaryworkspace.rcp.wwj.structures.IndexedPosition;
import com.binaryworkspace.rcp.wwj.structures.IndexedPositionColors;
...
/**
 * A NASA World Wind sample view that demonstrates how to draw surface paths.
 * <p>
 * <b>Notes:</b>
 * <ul>
 ...
 * <li>The algorithm for generating the parametric plot (path) is a modified
 * code example from Mathematica: ParametricPlot -> Neat Examples -> Forced
 * undersampling.
 * </ul>
 * 
 * @author Chris Ludka
 ...
 * @see <a href=
 *      "http://reference.wolfram.com/language/ref/ParametricPlot.html#23577">
 *      http://reference.wolfram.com/language/ref/ParametricPlot.html#23577</a>
 */
public class SurfacePathView extends ViewPart implements IScaleDoubleControlComposition, IScaleIntControlComposition {

	/**
	 * The ID of the view as specified by the extension.
	 */
	public static final String ID = SurfacePathView.class.getName();
	...
	private final String LAYER_NAME = "SURFACE PATH";
	...
	private final double INCREMENT_LAT = 0.1;
	...
	private final double INCREMENT_LON = 0.1;
	...
	private final int MAX_POINT_COUNT = 1000;
	 ...
	 * zero is specified.
	 */
	private final double MIN_SCALING_FACTOR = 10.0;

	private final double MAX_SCALING_FACTOR = 360.0;

	private final double INCREMENT_SCALING_FACTOR = 10.0;

	private double scalingFactor = 180.0;

	/**
	 * Terrain Conformance Control
	 * <p>
	 * All terrain conformance specifications are in meters. A default minimum
	 * terrain conformance of one is specified and a value of zero will throw an
	 * exception from the NASA World Wind API.
	 */
	private final double MIN_TERRAIN_CONFORMANCE = 1.0;

	private final double MAX_TERRAIN_CONFORMANCE = 100.0;

	private final double INCREMENT_TERRAIN_CONFORMANCE = 0.5;

	private double terrainConformance = 1.0;
	...
	public void createPartControl(Composite parent) {
		...
		// LHS: LatLon Control Composite
		Composite latLonControlComposite = new Composite(lhs, SWT.NONE);
		latLonControlComposite.setBackground(SwtColors.color(ColorType.BACKGROUND_DARK));
		latLonControlComposite.setLayout(new FormLayout());
		formData = new FormData();
		formData.top = new FormAttachment(0, Margins.TOP_MARGIN.margin());
		formData.left = new FormAttachment(0, Margins.LEFT_MARGIN.margin());
		formData.right = new FormAttachment(100, Margins.RIGHT_MARGIN.margin());
		latLonControlComposite.setLayoutData(formData);
		...
		// LHS: Terrain Conformance Composite
		Composite terrainConformanceComposite = new Composite(lhs, SWT.NONE);
		terrainConformanceComposite.setBackground(SwtColors.color(ColorType.BACKGROUND_DARK));
		terrainConformanceComposite.setLayout(new FormLayout());
		formData = new FormData();
		formData.top = new FormAttachment(scalingFactorComposite, Margins.TOP_MARGIN.margin());
		formData.left = new FormAttachment(0, 0);
		formData.right = new FormAttachment(100, 0);
		terrainConformanceComposite.setLayoutData(formData);

		// LHS: Terrain Conformance Scale
		ScaleDoubleControlComposition terrainConformanceScale = new ScaleDoubleControlComposition( //
				terrainConformanceComposite, // Parent
				this, // Ref
				MIN_TERRAIN_CONFORMANCE, // Min Value
				MAX_TERRAIN_CONFORMANCE, // Max Value
				INCREMENT_TERRAIN_CONFORMANCE, // Increment
				terrainConformance, // Initial Value
				ScaleDoubleType.TERRAIN_CONFORMANCE // Scale Double Type
		); //
		terrainConformanceScale.init();
	...
	private void refresh() {
		// Build Positions
		List<IndexedPosition> positionList = new ArrayList<IndexedPosition>(pointCount);
		for (int i = 0; i < pointCount; i++) {
			/**
			 * The algorithm for generating the parametric plot (path) is a
			 * modified code example from Mathematica: ParametricPlot -> Neat
			 * Examples -> Forced undersampling.
			 * 
			 * <pre>
			* &#64;see http://reference.wolfram.com/language/ref/ParametricPlot.html#23577
			 * </pre>
			 */
			double pathLat = lat + (float) (Math.sin(i) * i) / scalingFactor;
			double pathLon = lon - (float) (Math.cos(i) * i) / scalingFactor;
			LatLon pathLatLon = LatLon.fromDegrees(pathLat, pathLon);
			positionList.add(new IndexedPosition(pathLatLon, 0.0f, i));
		}

		// Build Path
		Path path = new Path(positionList);
		path.setValue(AVKey.DISPLAY_NAME, LAYER_NAME);
		path.setPathType(AVKey.LINEAR);
		path.setAltitudeMode(WorldWind.RELATIVE_TO_GROUND);
		path.setPositionColors(new IndexedPositionColors(pointCount));
		path.setShowPositions(true);
		path.setFollowTerrain(true);
		path.setTerrainConformance(terrainConformance);
