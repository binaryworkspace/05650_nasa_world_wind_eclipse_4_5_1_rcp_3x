 ...
 * 100,000's of random points clamped to the ground. All points will share the
 * same color at any given time. After a small delay a new set of random points
 * with a new color will be generated.
 * <p>
 * <b>Notes:</b>
 * <ul>
 ...
 * @see <a href=
 *      "https://github.com/nasa/World-Wind-Java/blob/master/WorldWind/src/gov/nasa/worldwindx/examples/lineofsight/GridOfPoints.java">
 *      https://github.com/nasa/World-Wind-Java/blob/master/WorldWind/src/gov/
 *      nasa/worldwindx/examples/lineofsight/GridOfPoints.java</a>
 * @see <a href=
 *      "https://github.com/nasa/World-Wind-Java/blob/master/WorldWind/src/gov/nasa/worldwindx/examples/lineofsight/PointGrid.java">
 *      https://github.com/nasa/World-Wind-Java/blob/master/WorldWind/src/gov/
 *      nasa/worldwindx/examples/lineofsight/PointGrid.java</a>
 */
public class PointRandomizationView extends ViewPart implements IScaleDoubleControlComposition, IScaleIntControlComposition, IAnimationControlComposition {

	/**
	 * The ID of the view as specified by the extension.
	 */
	public static final String ID = PointRandomizationView.class.getName();
	...
	private final String LAYER_NAME = "POINT RANDOMIZATION";
	...
	private final int MAX_POINT_COUNT = 500000;
	...
	public void refreshFrameIndex(int frameIndex) {
		// NOTE: 'frameIndex' is not currently used
		refresh();
	}
	...
	private void refreshCornerPositions(){
		...
		cornerPositionList.add(Position.fromDegrees(lowerLat, lowerLon, ALT));
		cornerPositionList.add(Position.fromDegrees(lowerLat, upperLon, ALT));
		cornerPositionList.add(Position.fromDegrees(upperLat, upperLon, ALT));
		cornerPositionList.add(Position.fromDegrees(upperLat, lowerLon, ALT));		
	}
	
	// Refreshes the Point Randomizations
	private void refresh() {
		// Point positions
		List<Position> randPositions = new ArrayList<Position>(pointCount);
		double lat;
		double lon;
		double elev = 0.0;
		for (int i = 0; i < pointCount; i++) {
			// Add position
			lat = lowerLat + Math.random() * (upperLat - lowerLat);
			lon = lowerLon + Math.random() * (upperLon - lowerLon);
			randPositions.add(Position.fromDegrees(lat, lon, elev));
		}
		
		// Create the PointGrid shape
		ColorizedPointGrid grid = new ColorizedPointGrid(cornerPositionList, randPositions, randPositions.size());
		ColorizedPointGrid.Attributes attrs = new ColorizedPointGrid.Attributes();
		attrs.setPointSize(pointSize);
		attrs.setPointColor(AwtColorScheme.getVisibleSpectrum((float) Math.random()));
		grid.setAttributes(attrs);
		grid.setAltitudeMode(WorldWind.CLAMP_TO_GROUND);
